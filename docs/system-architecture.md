...
## 1. 组件关系
    ...
    │   └── agent-openclaw.service（3081 预留）
    ├── Model service
...
DSH、未来新增的 Agent 和其他 OpenAI-compatible 客户端共享本地模型 API。
Agent 不直接加载 GGUF，模型服务也不负责工作区、文件或工具编排。
...
- 运行时：Node.js v24.19.0、pnpm v10.28.0；
...
| 3081 | OpenClaw | K3 127.0.0.1，预留 | 后续 Agent |
...
| SSH forwarding | 访问设备 → K3 | 访问设备本地回环 | 浏览器访问 DSH |
...
- Restart=on-failure 负责进程异常后的自动拉起；
- scripts/start.sh 等待 8080 健康后再启动 DSH；
- systemd journal 保存服务日志，fleet 写操作另通过 logger 记录审计信息。

## 6. 安全边界

- 3080、8080 和未来 Agent 端口默认只监听 K3 的 127.0.0.1；
- 访问 DSH 使用 SSH 隧道，不直接开放服务端口；
